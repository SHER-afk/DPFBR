...
class Engine(object):
    ...
    def fed_train_single_batch(self, model_client, batch_data, optimizers, user):
        ...
        regularization_term = compute_item_regularization(model_client, reg_item_embedding)
    ...
    def aggregate_clients_params(self, round_user_params):
        ...
        print(f"Memory used by my_function: {memory_used / 1024 / 1024} MB")
        return memory_used
    ...
    def fed_train_a_round(self, all_train_data, round_id):
        ...
        for user in participants:
            ...
            # update client model.
            """if user==0:
                size1 = asizeof.asizeof(self.server_model_param['embedding_item.weight']['global'])
                size2 = asizeof.asizeof(self.server_model_param['embedding_item.weight'][0])
                current = size1 + size2
                print(f"x 的真实占用内存: {current} 字节")"""
        ...
        memory_used=self.aggregate_clients_params(round_participant_params)
        return participants,memory_used
