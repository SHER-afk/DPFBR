...
import random
import copy
from data import UserRatingDataset
...
class Engine(object):
    ...
    def __init__(self, config):
        ...
        # implicit feedback
        self.crit = torch.nn.BCELoss()#是用于二分类问题的损失函数，适合处理输出为概率（例如 Sigmoid 激活后）的二分类任务。它计算预测的概率分布与真实标签之间的交叉熵误差。
        # mae metric
        self.mae = torch.nn.L1Loss()

    def instance_user_train_loader(self, user_train_data):
        """instance a user's train loader."""
        dataset = UserRatingDataset(user_tensor=torch.LongTensor(user_train_data[0]),
    ...
    def fed_train_a_round(self, all_train_data, round_id):
        ...
        #select a client set of size n randomly from all N clients
        ...
        for user in participants:
            ...
            model_client = copy.deepcopy(self.model)#复制一个
            ...
            if round_id != 0:
                ...
                #print("user_param_dict\n",user_param_dict)
                if user in self.client_model_params.keys():
                    #print("self.client_model_params[user]\n",self.client_model_params[user].keys())
                ...
                #print("\n#\n",self.server_model_param['embedding_item.weight'].data,"\n")
            ...
            optimizer_i = torch.optim.SGD(model_client.embedding_item.parameters(),
                                          lr=self.config['lr'] * self.config['num_bundles'] * self.config['lr_eta'],
            ...
            user_train_data = [all_train_data[0][user], all_train_data[3][user], all_train_data[4][user]]
    ...
    def fed_evaluate(self, evaluate_data):
        ...
        for user in range(user_num):
            ...
            with torch.no_grad():
                ...
                # 模型预测正样本
                pos_score = user_model(pos_bundle)
                ...
                neg_score = user_model(neg_bundle)
